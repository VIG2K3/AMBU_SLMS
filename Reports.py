...
class ReportSystem(QWidget):
    def __init__(self):
        ...
        self.tabs.addTab(self.history_tab, "Reports")
    ...
    def init_history_tab(self):
        ...
        self.search_input.setPlaceholderText("Enter category or product name")
        ...
        """)
        self.search_input.textChanged.connect(self.filter_history_items)

        export_button = QPushButton("Export to Excel")
        export_button.setFixedSize(160, 35)
    ...
    def move_selected_item(self):
        ...
        confirm = QMessageBox.question(self, "Confirm Transfer",
                                       f"Are you sure you want to move '{name}' to reports?\n(Action irreversible)",
    ...
    def filter_history_items(self):
        ...
        for row in range(self.history_table.rowCount()):
            match = False
            for col in [0, 1]:  # category and name
    ...
    def export_to_excel(self):
        path, _ = QFileDialog.getSaveFileName(self, "Save Report", "report.xlsx", "Excel Files (*.xlsx)")
        if path:
            try:
                wb = Workbook()
                ws = wb.active
                ...
                # Define headers
                headers = [self.history_table.horizontalHeaderItem(i).text() for i in
                           range(self.history_table.columnCount())]
                ws.append(headers)

                # Apply bold and center alignment to header
                ...
                # Style each cell and auto-fit columns
